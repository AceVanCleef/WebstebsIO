...
namespace Stebs5
{
    ...
    public class StebsHub : Hub
    {
        ...
        private void InitialiseClient(Guid guid)
        {
            Clients.Caller.Initialise(new InitialiseViewModel(
            ...
            foreach (var entry in Mpm.Instructions)
            {
                IInstruction ins = entry.Value;
                code.Add(ins.OpCode);
                addr.Add(ins.MpmAddress);
                mm.Add(ins.Mnemonic);
                List<string> s = new List<string>();
                foreach (var o in ins.OperandTypes)
                {
                    s.Add(o.ToString());
                }
                op.Add(s.ToArray());
            ...
            Clients.Caller.setOpDecoder(code.ToArray(), addr.ToArray(), mm.ToArray(), op.ToArray());

            List<int> adr = new List<int>();
            List<string> na = new List<string>();
            List<bool> ci = new List<bool>();
            List<bool> ev = new List<bool>();
            ...
            foreach (var entry in Mpm.MicroInstructions)
            {
                IMicroInstruction instruction = entry.Value;
                adr.Add(instruction.Address);
                na.Add(instruction.NextAddress.ToString());
                ci.Add(instruction.ClearInterrupt);
                ev.Add(instruction.EnableValue);
                jc.Add(instruction.JumpCriterion.ToString());
                val.Add(instruction.Value);
                af.Add(instruction.AffectFlags);
                alu.Add(instruction.AluCommand.ToString());
                io.Add(instruction.DataInput.ToString());
                src.Add(instruction.Source.ToString());
                dest.Add(instruction.Destination.ToString());
                rw.Add(instruction.ReadWrite.ToString());
            }
            Clients.Caller.setMpm(adr.ToArray(), na.ToArray(), ci.ToArray(), ev.ToArray(), jc.ToArray(), val.ToArray(), af.ToArray(), alu.ToArray(), io.ToArray(), src.ToArray(), dest.ToArray(), rw.ToArray());
        }
        ...
        public void CreateBreakpoints(int[] breakpointArray) => Manager.CreateBreakpoints(Context.ConnectionId, breakpointArray);
        ...
        public void ChangeStepSize(SimulationStepSize stepSize) => DoWithCheckedStepSize(stepSize, s => Manager.ChangeStepSize(Context.ConnectionId, s));
        ...
        public void UndoStep(SimulationStepSize stepSize) => DoWithCheckedStepSize(stepSize, s => Manager.UndoStep(Context.ConnectionId, s));
        ...
        /// <param name="slot">Device slot in the processor.</param>
        /// <param name="input">Update information.</param>
        /// <param name="input">second Update information.</param>
        public void UpdateDevice(byte slot, string input, string secondInput) => Manager.UpdateDevice(Context.ConnectionId, slot, input, secondInput);

        public void HwInterrupt() => Manager.HardwareInterrupt(Context.ConnectionId);
    }
}
