...
namespace ProcessorDispatcher
{
    public class Dispatcher : IDispatcher
    {
        ...
        private Action<IDispatcherItem, SimulationStepSize, int, AluCmd, IReadOnlyDictionary<byte, byte>, IReadOnlyDictionary<Registers, IRegister>, bool[]> finishedStep;
        public event Action<IDispatcherItem, SimulationStepSize, int, AluCmd, IReadOnlyDictionary<byte, byte>, IReadOnlyDictionary<Registers, IRegister>, bool[]> FinishedStep
        {
            add
            {
        ...
        private Action<IDispatcherItem> historyStackEmpty;
        public event Action<IDispatcherItem> HistoryStackEmpty
        {
            add
            {
                lock (eventLock) { historyStackEmpty += value; }
            }
            remove
            {
                lock (eventLock) { historyStackEmpty -= value; }
        ...
        private void NotifyFinishedStep(IDispatcherItem item, SimulationStepSize stepSize, int address, AluCmd command, IReadOnlyDictionary<byte, byte> ramChanges, IReadOnlyDictionary<Registers, IRegister> registerChanges, bool[] statusRegisterChanges)
        {
            Action<IDispatcherItem, SimulationStepSize, int, AluCmd, IReadOnlyDictionary<byte, byte>, IReadOnlyDictionary<Registers, IRegister>, bool[]> handler;
            lock (eventLock)
            {
            ...
            if (handler != null)
            {
                ...
                handler(item, stepSize, address, command, ramChanges, registerChanges, statusRegisterChanges);
            }
        }

        /// <summary>Notifies, when a reset, halt or paused request finished execution.</summary>
        ...
        private void NotifyHistoryStackEmpty(IDispatcherItem item)
        {
            Action<IDispatcherItem> handler;
            lock (eventLock)
            {
                handler = historyStackEmpty;
            }
            if (handler != null)
            {
                handler(item);
        ...
        public void UndoStep(Guid id, SimulationStepSize stepSize)
        {
            switch (stepSize)
            {
                case SimulationStepSize.Micro:
                    stepSize = SimulationStepSize.UndoMicro;
                    break;
                case SimulationStepSize.Macro:
                    stepSize = SimulationStepSize.UndoMacro;
                    break;
                case SimulationStepSize.Instruction:
                    stepSize = SimulationStepSize.UndoInstruction;
                    break;
            }
        ...
        private void Execute()
        {
            lock (executionLock)
            {
                ...
                foreach(var execution in executions)
                {
                    ...
                    try
                    {
                        switch (execution.Value)
                        {
                            ...
                            case SimulationStepSize.Micro:
                                ...
                                break;
                            case SimulationStepSize.UndoInstruction:
                                simulator.UndoInstructionStep(item.Processor);
                                break;
                            case SimulationStepSize.UndoMacro:
                                simulator.UndoMacroStep(item.Processor);
                                break;
                            case SimulationStepSize.UndoMicro:
                                simulator.UndoMicroStep(item.Processor);
                                break;
                    ...
                    collector.Unbind();
                    NotifyFinishedStep(item, execution.Value, collector.MpmAddressChange, collector.AluCommandChange, collector.RamChanges, collector.RegisterChanges, collector.StatusRegisterChanges);
                    ...
                    if (item.Processor.HandleBreakpoint())
                    {
                        Update(item.Guid, i => i.SetRunning(false));
                        NotifyStateChanged(item, StateChange.Pause);
                    }

                    if (item.Processor.IsHistoryStackEmpty())
                    {
                        NotifyHistoryStackEmpty(item);
                    }
                }
            }
#if DEBUG
